...
def build_instance_graph(
    ...
    for p in products.list_all():
        ...
        # Brand individuals are deduped by name so multiple products from the
        # same brand (e.g. two Northwind laptops) point at one Brand node.
        if p.brand not in brand_nodes:
            brand_node = RETAIL[f"Brand/{p.brand.replace(' ', '_')}"]
            g.add((brand_node, RDF.type, RETAIL.Brand))
            g.add((brand_node, RDFS.label, Literal(p.brand)))
            g.add((brand_node, RETAIL.hasBrandName, Literal(p.brand, datatype=XSD.string)))
            brand_nodes[p.brand] = brand_node
        g.add((node, RETAIL.hasBrand, brand_nodes[p.brand]))
...
def low_stock_products(g: Graph, threshold: int) -> list[dict]:
    """SPARQL query: products whose InventoryItem.hasQuantityAvailable is
    below `threshold` -- a plain FILTER, shown here as a contrast to the
    property-path query in products_in_category_tree above."""
    query = """
    PREFIX retail: <http://retail-ddd-demo.org/onto#>
    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    SELECT ?product ?label ?available WHERE {
        ?product a retail:Product ;
                 rdfs:label ?label ;
                 retail:hasInventory ?inv .
        ?inv retail:hasQuantityAvailable ?available .
        FILTER (?available < %d)
    }
    """ % threshold
    rows = g.query(query)
    return [
        {"product_uri": str(r.product), "label": str(r.label), "quantity_available": int(r.available)}
        for r in rows
    ]
